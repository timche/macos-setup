...
# logged in, or a token without the scope. On a fresh Mac all of those are true
# when install.sh reaches this, and the run should carry on regardless — you come
...
gh ssh-key add "$signing_key" --type signing --title "$(hostname -s)"
